...
def get_forecasts(models, horizon, X_test: pd.DataFrame=None, ensemble: bool=True) -> Dict:
    """
    Produce a dictionary with keys the model names and values as the model predictions
    """
    n = len(models)
    forecasts = {
        name: model.predict(horizon, X=X_test) for name, model in models.items()
    }
    if ensemble:
...
def get_metrics_results(metrics: Dict, forecasts: Dict, actual: pd.Series) -> pd.DataFrame:
    """
